...
class GradeController {
    ...
    async IndexActivitiesDaily(req, res) {
        ...
        try {
            ...
            const activit = await Activities.find(filter)
                .populate('studentGrades')
                .populate('id_teacher')
            ...
            console.log('Resultados encontrados no banco:', activit);

            // Filtra os resultados pela `year` (convertendo para string) e `id_class`
            const Activit = activit.filter(res => {
            ...
            console.log('Resultados após filtro:', Activit);
